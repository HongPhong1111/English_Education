...
import { questionApi, QuestionResponse, QuestionRequest } from '../../services/api/questionApi'
...
export default function QuestionsPage() {
    ...
    const openCreate = () => {
        setEditingId(null)
        setForm({ ...emptyForm, lessonId: filterLesson ? parseInt(filterLesson) : '' })
        setDialogOpen(true)
    }
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (item: Record<string, unknown>) => {
                ...
                const variant = type === 'MULTIPLE_CHOICE' ? 'info' : type === 'TRUE_FALSE' ? 'success' : 'warning'
        ...
        },
        {
            ...
            label: 'Điểm',
            render: (item: Record<string, unknown>) => (
                <span style={{ color: 'var(--color-text-secondary)' }}>{(item.points as number) ?? 1}</span>
            ),
        },
        {
            key: 'lessonTitle',
            label: 'Bài học',
            render: (item: Record<string, unknown>) => (
                <span style={{ color: 'var(--color-text-secondary)' }}>{(item.lessonTitle as string) || '—'}</span>
            ),
        },
        {
            key: 'actions',
