...
export default function ClassRoomEditPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoadingData(true)
            try {
                // Fetch classroom, schools and teachers
                const [classRes, schoolsRes, usersRes] = await Promise.allSettled([
                    api.get<ApiResponse<ClassRoom>>(`/classes/${id}`),
                    api.get<ApiResponse<SchoolType[]>>('/schools'),
                    api.get<ApiResponse<Page<User>>>('/users', { params: { page: 0, size: 1000 } }),
                ])

                if (classRes.status === 'fulfilled') {
                    const room = classRes.value.data.data
                    setForm({
                        name: room.name,
    ...
    return (
        <div className="space-y-6 max-w-2xl mx-auto pb-10">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-black tracking-tight">Chỉnh sửa lớp học</h1>
                    <p className="text-muted-foreground font-medium">Cập nhật thông tin chi tiết của lớp học.</p>
                </div>
            </div>
